...
powershell -Command "Start-Process powershell -Verb RunAs -Wait -ArgumentList '-ExecutionPolicy Bypass -File ""%~dp0tools\installer\clean_install.ps1""'"

echo.
echo [2/2] Configuring Windows Language Switcher (Win + Space) for current user...
powershell -ExecutionPolicy Bypass -File "%~dp0tools\installer\enable_tip.ps1"
